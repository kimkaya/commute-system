...
async function handleLogin(e) {
    ...
    try {
        ...
        messageDiv.innerHTML = '<div class="alert alert-error">로그인 중 오류가 발생했습니다.</div>';
    }
}
...
async function loadDashboard() {
    try {
        ...
        // 오늘 출퇴근 기록 로드
        const today = new Date().toISOString().split('T')[0];
        const recordsResponse = await fetch(`/api/records?startDate=${today}&endDate=${today}`);
        ...
        const monthRecordsResponse = await fetch(`/api/records?startDate=${firstDay}&endDate=${today}`);
...
async function editEmployee(name) {
    ...
    if (newHourlyRate !== null && !isNaN(newHourlyRate)) {
        try {
            const response = await fetch(`/api/employees/${name}/schedule`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
